...
describe('Sequencer', () => {
    ...
    it('schedules outgoing ping for first note', () => {
        const pingStub = {
            start: jest.fn(),
            stop: jest.fn(),
        };
        ...
        const outPlugs: OutAudioPlugValues = {
            ping: {
                Ping: {connected: true, value: pingStub as Ping},
            },
            number: {
                Frequency: {connected: false, value: undefined},
            },
        ...
        expect(pingStub.start).toHaveBeenCalledTimes(1);
        expect(pingStub.stop).toHaveBeenCalledTimes(1);
        expect(pingStub.start).toHaveBeenCalledWith(0);
        expect(pingStub.stop).toHaveBeenCalledWith(0.25);
    });

    it('schedules outgoing pings for many notes, with large lookahead', () => {
        const pingStub = {
            start: jest.fn(),
            stop: jest.fn(),
        };
        ...
        render(<SequencerWithState 
            mutableState={SequencerDefinition.initializeMutableState({ globalAudioContext, lookahead: 1000 })}
            outPlugs={outPlugs}
            applicationContext={{ globalAudioContext, lookahead: 1000 }}
        />);
        ...
        expect(pingStub.start).toHaveBeenCalledTimes(4);
        expect(pingStub.stop).toHaveBeenCalledTimes(4);

        expect(pingStub.start.mock.calls[0][0]).toEqual(0);
        expect(pingStub.start.mock.calls[1][0]).toEqual(0.25);
        expect(pingStub.start.mock.calls[2][0]).toEqual(0.5);
        expect(pingStub.start.mock.calls[3][0]).toEqual(0.75);

        expect(pingStub.stop.mock.calls[0][0]).toEqual(0.25);
        expect(pingStub.stop.mock.calls[1][0]).toEqual(0.5);
        expect(pingStub.stop.mock.calls[2][0]).toEqual(0.75);
        expect(pingStub.stop.mock.calls[3][0]).toEqual(1);
    });
});